...
for k in range(num):
   ...
   aVals_16 = []
   bVals_16 = []
   with open("k_of_" + sys.argv[1] + ".csv", 'rb') as csvfile:
       ...
       for row in spamreader:	
           ...
           if(i != 0):
              ...
              if(row[k+2] != ' '):
                 ...
                 zVals_15.append(float(row[k+2]))
              #print(k)
              #print(row)	
              if(row[k+2+num] != ' '):
                 aVals_16.append(float(row[0]))  
                 bVals_16.append(float(row[1]))  
                 zVals_16.append(float(row[k+2+num]))
           i+=1
...
for k in range(num):           #c, m, zlow, zhigh in [('r', 'o', -50, -25), ('b', '^', -30, -5)]:
   xs = np.array(aVals_15_full[k])
   ys = np.array(bVals_15_full[k])
   zs = np.array(zVals_15_full[k])
   ''' 
   print(k)
   ...
   ax.scatter(xs, ys, zs, c=color_array[k], marker=marker_array[k])
...
for k in range(num):           #c, m, zlow, zhigh in [('r', 'o', -50, -25), ('b', '^', -30, -5)]:
   xs = np.array(aVals_16_full[k])
   ys = np.array(bVals_16_full[k])
   zs = np.array(zVals_16_full[k])
   ...
   ae.scatter(xs, ys, zs, c=color_array[k], marker=marker_array[k])
